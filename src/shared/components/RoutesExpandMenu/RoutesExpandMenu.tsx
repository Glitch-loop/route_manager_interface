...
interface RoutesExpandMenuProps {
  ...
  open: boolean;
  onClose: () => void;
  mapStores?: Map<string, LocationDTO>; // Optional map of store ID to StoreDTO for quick access
...
export default function RouteExpandMenu({
  ...
  const [hoveredDayTooltip, setHoveredDayTooltip] = React.useState<
    string | null
  >(null);
  ...
  const handleDayClick = (dayId: string) => {
    if (showDayCheckbox) {
      ...
      if (onDaySelectCheckbox !== undefined) {
        console.log(
          `Day ${dayId} checkbox clicked. New state: ${newCheckedState}`,
        );
        onDaySelectCheckbox(dayId, newCheckedState);
      }
    } else {
  ...
  const getTooltipText = (stores: RouteDayLocationDTO[]) => {
    let zonification: string = "";
    const identifiedZones: Set<string> = new Set();

    if (mapStores !== undefined) {
      stores.forEach((store: RouteDayLocationDTO) => {
        const storeInfo = mapStores.get(store.id_location);
        if (storeInfo) {
      ...
      if (identifiedZones.size > 0) {
        ...
        identifiedZones.forEach((zone: string) => {
          zonification = zonification.concat(
            capitalizeFirstLetterOfEachWord(zone) + ", ",
          );
        });
            ...
            >
              <ClickAwayListener onClickAway={() => {}}>
                <MenuList sx={{ padding: 0 }}>
                  ...
                  >
                    <span style={{ width: 90 }}>
                      {capitalizeFirstLetterOfEachWord("Dia")}
                    </span>
                    <span style={{ width: 70, textAlign: "center" }}>
                      {capitalizeFirstLetterOfEachWord("Clientes")}
                    </span>
                    {showDayCheckbox && (
                      <span style={{ width: 70, textAlign: "center" }}>
                        {capitalizeFirstLetterOfEachWord("Selected")}
                      </span>
                    )}
                  </MenuItem>
                  {/* Day rows */}
                  {DAYS_ARRAY.map((day) => {
                    ...
                    const { day_name } = day;
                    const { route_day } = activeRoute;
                    
                    const routeDayFound = route_day.find((routeDay) => routeDay.id_day === day.id_day);
                    

                    if (routeDayFound === undefined) return null;

                    const { id_route_day, locations } = routeDayFound;

                    return (
                      <Tooltip
                        key={id_route_day}
                        title={getTooltipText(locations)}
                        open={
                          hoveredDayTooltip === id_route_day &&
                          mapStores !== undefined
                        }
                        placement="right"
                        arrow
                      >
                        <MenuItem
                          ...
                          onMouseEnter={() =>
                            handleDayHover(id_route_day, day_name)
                          }
                        ...
                        >
                          <span style={{ width: 90 }}>
                            {capitalizeFirstLetterOfEachWord(day_name)}
                          </span>
                          <span
                            style={{
                              width: 70,
                          ...
                          {showDayCheckbox && (
                            <span style={{ width: 70, textAlign: "center" }}>
                              <Checkbox
                                ...
                                onClick={(e) =>
                                  handleCheckboxChange(id_route_day, e)
                                }
                                size="small"
                                sx={{ padding: 0 }}
                              />
