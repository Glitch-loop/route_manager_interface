...
function createMapClickComponent(
  ...
  onSelectStoreToUpdate?: (idStore: string) => void,
): React.ReactNode {
    ...
    <div className="p-3 min-w-[280px]">
      {onSelectStoreToUpdate && (
        <div className="flex items-center justify-between mb-2">
          ...
          >
            <IconButton
              onClick={() => onSelectStoreToUpdate(id_location)}
              sx={{
...
export default function Page() {
        ...
        if (store !== undefined) {
          ...
          markers.push({
            ...
            clickComponent: createMapClickComponent(store, storePositions, () =>
              handleSelectStoreToUpdate(id_location),
            ),
          });
        }
      });
  ...
  const handleDesactivateStore = async (idLocation: string) => {
    ...
    try {
      ...
      setStores((prevStores) =>
        prevStores.filter((store) => store.id_location !== idLocation),
      );
    } catch (error) {
      console.error("Error updating store: ", error);
    }
  ...
  const handleRouteDaySelect = (routeDayId: string, state: boolean) => {
    if (state) {
      ...
      // Show confirmation dialog before unselecting
      let askValidation = false;
      const routeDayFound: RouteDayDTO | null = getRouteDayFromRoutesList(
        routes,
        routeDayId,
      );
      const routeDayBeingModified: DraggableRouteDayStore[] | undefined =
        routesInModification[routeDayId];

      if (routeDayFound === null || routeDayBeingModified == undefined) return;

      const locationOfRouteDay = routeDayFound.locations;

      if (locationOfRouteDay.length === routeDayBeingModified.length) {
        for (let i = 0; i < locationOfRouteDay.length; i++) {
          const store = locationOfRouteDay[i];
          const storeBeingModified = routeDayBeingModified[i];

          if (
            store.id_route_day_store !== storeBeingModified.id_route_day_store
          )
            askValidation = true;
        }
      } else {
        // It means there are changes in the store of the route day.
        askValidation = true;
      }
  ...
  const handleSaveRouteModification = async (
  ...
  return (
    <div className="h-full w-full overflow-hidden flex flex-row bg-system-primary-background rounded-lg">
      ...
      <Dialog
        open={pendingUnselectRouteDayId !== null}
        onClose={handleCancelUnselectRouteDay}
      >
        <div className="p-5 flex flex-col gap-2 justify-center items-center">
          <h3 className="text-center font-bold text-lg">
            ¿Estas seguro de deseleccionar este día de ruta?
          </h3>
          <span className="text-center text-red-600 font-semibold">
            Se cerrará la modificación de este día de ruta. Cualquier cambio no
            guardado se perderá.
          </span>
          ...
          <div className="flex flex-col h-full">
            <div className="w-full flex items-center justify-center p-4">
              <ButtonGroup variant="contained">
                <Button onClick={() => handleAdministrationView(1)}>
                  Rutas
                </Button>
                <Button onClick={() => handleAdministrationView(2)}>
                  Clientes
                </Button>
              </ButtonGroup>
            </div>
                ...
                <List className="max-h-[40vh] min-h-[30vh] overflow-y-auto">
                  {storeWithinRouteAssigned.size === 0 && (
                    <p className="text-sm text-gray-500 italic text-center">
                      Todas las tiendas están asignadas a una ruta.
                    </p>
                  )}
                  {Array.from(storeWithinRouteAssigned.entries()).map(
                    ([id_store]) => {
                      const store = mapStores.get(id_store);
                      if (store === undefined) return null;
                      const { location_name } = store;
                      return (
                        <ListItem key={id_store}>
                          <SimpleCard
                            cardName={
                              capitalizeFirstLetterOfEachWord(location_name) ??
                              "Nombre no disponible"
                            }
                            cardDetails={capitalizeFirstLetterOfEachWord(
                              getAddressOfStore(store),
                            )}
                          />
                        </ListItem>
                      );
