...
// Controllers
import { getStoresByDate } from "@/controllers/WorkDayController";
...
import {
  getInventoryOperationsOfWorkDay,
  getInventoryOperationDescriptionsOfWorkDay,
...
// Actions
import { listRouteTransactionsWithinDateRange } from "@/shared/actions/transactionActions";
...
function getRouteName(idRoute: string, routes: IRoute[]): string {
  ...
  const index: number = routes.findIndex((route) => {
    return route.id_route === idRoute;
  });
...
function getVendorName(idVendor: string, vendors: IUser[]): string {
  ...
  const index: number = vendors.findIndex((vendor) => {
    return vendor.id_vendor === idVendor;
  });
...
function ConsultInformation() {
  ...
  const [routeTransactions, setRouteTransactions] = useState<
    IRouteTransaction[] | undefined
  >(undefined);
  const [routeTransactionOperations, setRouteTransactionOperations] = useState<
    IRouteTransactionOperation[] | undefined
  >(undefined);
  const [
    routeTransactionOperationDescriptions,
    setRouteTransactionOperationDescriptions,
  ] = useState<IRouteTransactionOperationDescription[] | undefined>(undefined);

  // States related to product inventory
  const [inventoryOperations, setInventoryOperations] = useState<
    IInventoryOperation[] | undefined
  >(undefined);
  const [inventoryOperationDescriptions, setInventoryOperationDescriptions] =
    useState<IInventoryOperationDescription[] | undefined>(undefined);
  const [productsInventory, setProductsInventory] = useState<
    IProductInventory[] | undefined
  >(undefined);

  const [nameOfStores, setNameOfStores] = useState<string[] | undefined>(
    undefined,
  );
  const [productRepositionByStore, setProductRepositionByStore] = useState<
    IProductInventory[][] | undefined
  >(undefined);
  const [productSoldByStore, setProductSoldByStore] = useState<
    IProductInventory[][] | undefined
  >(undefined);
  const [productDevolutionByStore, setProductDevolutionByStore] = useState<
    IProductInventory[][] | undefined
  >(undefined);
  ...
  const handlerSearchWorkDays = async () => {
    ...
    setWorkDays(
      await getStoresByDate(
        initialDate.format("YYYY/MM/DD"),
        finalDate.add(1, "day").format("YYYY/MM/DD"),
      ),
    );
  ...
  const handlerSelectWorkDay = async (
    ...
    const routeTransactions: IRouteTransaction[] =
      await getRouteTransactionsFromWorkDay(workDay);
    const routeTransactionOperations: IRouteTransactionOperation[] =
      await getRouteTransactionOperationsFromWorkDay(routeTransactions);
    const routeTransactionOperationDescriptions: IRouteTransactionOperationDescription[] =
      await getRouteTransactionOperationDescriptionsFromWorkDay(
    ...
    const inventoryOperations: IInventoryOperation[] =
      await getInventoryOperationsOfWorkDay(workDay);
    const inventoryOperationDescriptions: IInventoryOperationDescription[] =
      await getInventoryOperationDescriptionsOfWorkDay(inventoryOperations);
    ...
    const informationOfStores: IStore[] = await getInformationOfStores(
      [...storesOfTheDay.entries()].map((item) => {
        return item[0];
      }),
    );
    ...
    setNameOfStores(
      informationOfStores.map((currentStore) => {
        return currentStore.store_name;
      }),
    );

    // Reposition concept
    const responseInventoryByStoreAndDevolutionOperation: (IStore & {
      productInventory: IProductInventory[];
    })[] = await getTotalInventoriesOfAllStoresByIdOperationType(
    ...
    setProductDevolutionByStore(
      responseInventoryByStoreAndDevolutionOperation.map((currentInventory) => {
    ...
    // Reposition concept
    const responseInventoryByStoreAndRepositionOperation: (IStore & {
      productInventory: IProductInventory[];
    })[] = await getTotalInventoriesOfAllStoresByIdOperationType(
    ...
    setProductRepositionByStore(
      responseInventoryByStoreAndRepositionOperation.map((currentInventory) => {
        const { productInventory } = currentInventory;
        return productInventory;
      }),
    );
    ...
    // Selling concept
    const responseInventoryByStoreAndSaleOperation: (IStore & {
      productInventory: IProductInventory[];
    })[] = await getTotalInventoriesOfAllStoresByIdOperationType(
    ...
    setProductSoldByStore(
      responseInventoryByStoreAndSaleOperation.map((currentInventory) => {
        const { productInventory } = currentInventory;
        return productInventory;
      }),
    );
        ...
        {workday !== undefined && (
          <div className="flex flex-col ml-2">
            <span className="text-3xl font-bold">
              {capitalizeFirstLetter(getRouteName(workday.id_route, routes))}
            </span>
            <span className="text-2xl">
              Fecha:{" "}
              {cast_string_to_timestamp_standard_format(workday.start_date)}
            </span>
            <span className="text-xl">
              Vendedor: {getVendorName(workday.id_vendor, vendors)}
            </span>
